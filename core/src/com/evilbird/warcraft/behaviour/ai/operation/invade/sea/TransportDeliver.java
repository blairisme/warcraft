 ...
 * tasks blackboard to a disembarkation point, delivering attackers to the
 * landmass containing their target.
 *
 * @author Blair Butterworth
 */
public class TransportDeliver extends ActionTask<SeaInvasionData>
{
    @Inject
    public TransportDeliver(ActionFactory factory) {
        super(factory);
    }
    ...
    protected Action getAction(ActionFactory factory) {
        ...
        GameObject disembarkPoint = data.getDisembarkationPoint();

        Action action = factory.get(MoveToItem);
        action.setSubject(transport);
        action.setTarget(disembarkPoint);
