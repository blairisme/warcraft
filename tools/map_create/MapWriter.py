...
def round_pixel(pixel, divisor):
    return round_down(pixel[0], divisor), \
           round_down(pixel[1], divisor), \
           round_down(pixel[2], divisor)


def index_image(image, colours):
    width, height = image.size
    for x in range(width):
        for y in range(height):
            location = (x, y)
            pixel = image.getpixel(location)
            pixel = round_pixel(pixel, 256 / colours)
...
if sys.argv[1] == "--help":
    ...
    tile_map = TileMap(mapImageFile, Tileset(tileImageFile, tileX, tileY), tileX, tileY)
    tile_map.write(os.path.splitext(mapImageFile)[0] + ".tmx")
